...
cek("halaman /dashboard tanpa login -> redirect /login", dash.status === 307 && dash.headers.get("location")?.endsWith("/login"), dash.status);
...
const klaimBody = {
  item_id: itemId, nama_pengklaim: "Budi Santoso", no_hp: "+62 812-3456-7890",
  waktu_kehilangan: "2026-09-20", lokasi_kehilangan: "Ruang Baca", ciri_barang: "Kulit hitam, ada gantungan",
};
const tanpaCaptcha = await req("POST", "/api/klaim", { json: klaimBody, ip: "10.1.0.1" });
cek("POST /api/klaim tanpa captcha -> 400", tanpaCaptcha.status === 400 && tanpaCaptcha.data.fieldErrors?.captcha_token, tanpaCaptcha);
...
cek("POST /api/klaim dengan captcha -> 201 + nomor", klaim.status === 201 && /^CLM-/.test(klaim.data.data?.nomor_klaim), klaim);
...
let status429 = 0;
for (let i = 0; i < 6; i++) {
  const r = await req("POST", "/api/klaim", { json: { ...klaimBody, captcha_token: CAPTCHA_UJI, no_hp: `08120000000${i}` }, ip: "10.9.9.9" });
  status429 = r.status;
}
...
const langsung = await fetch(`${SUPA}/rest/v1/rpc/ajukan_klaim`, {
  ...
  body: JSON.stringify({ p_item_id: itemId, p_nama_pengklaim: "X", p_no_hp: "081299999999", p_waktu_kehilangan: "2026-09-20", p_lokasi_kehilangan: "x", p_ciri_barang: "x" }),
});
...
const loginTanpaCaptcha = await req("POST", "/api/auth/login", { json: { email: "petugas@dpad.test", password: "petugas123" }, ip: "10.2.0.1" });
cek("login tanpa captcha -> 400", loginTanpaCaptcha.status === 400, loginTanpaCaptcha);

const salah = await req("POST", "/api/auth/login", { json: { email: "petugas@dpad.test", password: "salah123", captcha_token: CAPTCHA_UJI }, ip: "10.2.0.1" });
cek("login password salah -> pesan generik", salah.status === 400 && salah.data.error === "Email atau password salah.", salah);

let loginStatus = 0;
for (let i = 0; i < 6; i++) {
  loginStatus = (await req("POST", "/api/auth/login", { json: { email: "orang@lain.test", password: "x", captcha_token: CAPTCHA_UJI }, ip: `10.3.0.${i}` })).status;
}
cek("rate limit login per email: ke-6 -> 429 (walau IP beda)", loginStatus === 429, loginStatus);

const login = await req("POST", "/api/auth/login", { json: { email: "petugas@dpad.test", password: "petugas123", captcha_token: CAPTCHA_UJI }, ip: "10.2.0.2" });
cek("login benar -> 200 + cookie sesi", login.status === 200 && login.data.data.nama === "Siti Petugas" && jar.size > 0, login);
cek("GET /api/auth/sesi", (await req("GET", "/api/auth/sesi")).data.data?.email === "petugas@dpad.test");

// ---------------------------------------------------------------- barang
const fd = () => {
  const f = new FormData();
  f.set("nama_barang", "Botol minum"); f.set("kategori", "lainnya"); f.set("warna", "Biru");
  f.set("deskripsi", "Ada stiker nama"); f.set("lokasi_ditemukan", "Lobi"); f.set("tanggal_ditemukan", "2026-09-22");
  f.set("tampilkan_foto", "on");
  return f;
};
const fPalsu = fd(); fPalsu.set("foto", fotoBlob(Buffer.from("<?php echo 1; ?>"), "x.jpg"));
const palsu = await req("POST", "/api/petugas/barang", { form: fPalsu });
cek("upload file palsu (bukan gambar) -> 400 fieldErrors.foto", palsu.status === 400 && palsu.data.fieldErrors?.foto, palsu);

const fBaru = fd(); fBaru.set("foto", fotoBlob(jpg));
const baru = await req("POST", "/api/petugas/barang", { form: fBaru });
cek("POST /api/petugas/barang -> 201 + kode", baru.status === 201 && /^BLG-2026-\d{3}$/.test(baru.data.data?.kode_barang), baru);
const barangBaru = baru.data.data.id;

const detail = await req("GET", `/api/petugas/barang/${barangBaru}`);
cek("GET /api/petugas/barang/:id (dengan deskripsi & foto_url)", detail.status === 200 && detail.data.data.deskripsi === "Ada stiker nama" && detail.data.data.foto_url, detail);

const ubah = await req("PATCH", `/api/petugas/barang/${barangBaru}`, {
  json: { nama_barang: "Botol minum stainless", kategori: "lainnya", lokasi_ditemukan: "Lobi", tanggal_ditemukan: "2026-09-22", tampilkan_foto: false },
});
...
cek("GET /api/petugas/barang?status=tersimpan", (await req("GET", "/api/petugas/barang?status=tersimpan&cari=botol")).data.data.total === 1);

// ---------------------------------------------------------------- klaim
const daftarKlaim = await req("GET", `/api/petugas/klaim?status=menunggu&cari=${klaim.data.data.nomor_klaim}`);
cek("GET /api/petugas/klaim?cari=<nomor klaim>", daftarKlaim.status === 200 && daftarKlaim.data.data.klaim.length === 1, daftarKlaim);
const claimId = daftarKlaim.data.data.klaim[0].id;

cek("GET /api/petugas/klaim/:id (barang lengkap)", (await req("GET", `/api/petugas/klaim/${claimId}`)).data.data.barang.deskripsi !== undefined);
...
const setuju = await req("POST", `/api/petugas/klaim/${claimId}/verifikasi`, { json: { keputusan: "setujui", catatan: "Ciri cocok" } });
...
cek("barang publik jadi 'dalam_proses_klaim'", (await req("GET", `/api/barang/${itemId}`)).data.data.status_publik === "dalam_proses_klaim");

// ---------------------------------------------------------------- serah terima
const tanpaFoto = new FormData(); tanpaFoto.set("persetujuan_foto", "on");
cek("serah terima tanpa foto -> 400", (await req("POST", `/api/petugas/klaim/${claimId}/serah-terima`, { form: tanpaFoto })).status === 400);

const tanpaSetuju = new FormData(); tanpaSetuju.set("foto", fotoBlob(jpg));
cek("serah terima tanpa persetujuan foto -> 400", (await req("POST", `/api/petugas/klaim/${claimId}/serah-terima`, { form: tanpaSetuju })).status === 400);

const st = new FormData(); st.set("foto", fotoBlob(jpg)); st.set("persetujuan_foto", "on"); st.set("catatan", "Diserahkan langsung");
...
cek("GET /api/petugas/pengembalian/:id", retDetail.status === 200 && retDetail.data.data.petugas.nama === "Siti Petugas", retDetail);
cek("  foto bukti bisa dibuka lewat signed URL", (await fetch(retDetail.data.data.foto_url)).ok);
cek("GET /api/petugas/pengembalian", (await req("GET", "/api/petugas/pengembalian?dari=2026-01-01")).data.data.total === 1);
...
cek("GET /api/petugas/laporan (rentang terbalik ditukar)", lap.status === 200 && lap.data.data.klaim_selesai === 1, lap);
const pb = await req("GET", "/api/petugas/laporan/per-bulan?tahun=abc");
cek("GET /api/petugas/laporan/per-bulan (tahun tidak valid -> tahun ini)", pb.status === 200 && pb.data.data.length === 12, pb);

// ---------------------------------------------------------------- akun
cek("PATCH /api/petugas/akun/profil", (await req("PATCH", "/api/petugas/akun/profil", { json: { nama: "Siti Rahayu" } })).status === 200);
cek("  nama berubah", (await req("GET", "/api/auth/sesi")).data.data.nama === "Siti Rahayu");

const pwSalah = await req("POST", "/api/petugas/akun/password", { json: { password_lama: "keliru", password_baru: "BaruSekali99", konfirmasi_password: "BaruSekali99" } });
cek("ganti password dengan password lama salah -> 400", pwSalah.status === 400 && pwSalah.data.fieldErrors?.password_lama, pwSalah);
const pwLemah = await req("POST", "/api/petugas/akun/password", { json: { password_lama: "petugas123", password_baru: "pendek", konfirmasi_password: "pendek" } });
cek("password baru lemah -> 400", pwLemah.status === 400, pwLemah);
const pw = await req("POST", "/api/petugas/akun/password", { json: { password_lama: "petugas123", password_baru: "BaruSekali99", konfirmasi_password: "BaruSekali99" } });
...
const loginBaru = await req("POST", "/api/auth/login", { json: { email: "petugas@dpad.test", password: "BaruSekali99", captcha_token: CAPTCHA_UJI }, ip: "10.4.0.1" });
